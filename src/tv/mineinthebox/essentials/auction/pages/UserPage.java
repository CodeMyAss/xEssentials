...
public class UserPage {
	 ...
	 * @return String
	 */
	public String parseUserContent(String user, boolean loggedin, boolean isAdmin) {
		try {
			Class.forName("org.sqlite.JDBC");
			Connection con = DriverManager.getConnection("jdbc:sqlite:"+fileName+".db");
			Statement state = con.createStatement();
			ResultSet set = state.executeQuery("SELECT * FROM auction_items WHERE owner=" + user + " ORDER BY date DESC");
			state.close();
			con.commit();
			con.close();
			...
			if(set.isBeforeFirst()) {
				...
				build.append("<div id=\"empty\"/><h1>No recent items where found!</h1><p>no recent items where found inside the shop, this could mean or the database is wiped or either its a new Minecraft auction!</p></div>");
			}
			build.append("</div>");
