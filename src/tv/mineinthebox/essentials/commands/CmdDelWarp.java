...
public class CmdDelWarp {
	...
	private List<String> getwarps(CommandSender sender, String p, Boolean playerOnly) {
		...
		if(!playerOnly) {
			List<Warp> s = new ArrayList<Warp>(Arrays.asList(xEssentials.getWarps()));
			for(Warp warp : s) {
			...
			if(xEssentials.isEssentialsPlayer(sender.getName())) {
			...
			if(sender.hasPermission(PermissionKey.CMD_DELWARP.getPermission())) {
				if(args.length == 0) {
					...
					if(args[0].equalsIgnoreCase("help")) {
						...
						if(xEssentials.doesWarpExist(args[0].toLowerCase())) {
							Warp warp = xEssentials.getWarpByName(args[0].toLowerCase());
