					...
					if(args.length == 0) {
						...
						if(args[0].equalsIgnoreCase("help")) {
							...
							if(victem instanceof Player) {
								...
								if(!(TpaManager.containsKey(victem.getName()) || TpaManager.containsValue(victem.getName()))) {
									TpaManager.put(victem.getName(), sender.getName());
									Bukkit.getScheduler().scheduleSyncDelayedTask(xEssentials.getPlugin(), new Runnable() {

										@Override
										public void run() {
											if(TpaManager.containsKey(victemName)) {
												TpaManager.remove(victemName);
												if(sender instanceof Player) {
