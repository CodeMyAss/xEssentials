...
public class PlayerChatHighLightEvent extends PlayerEvent implements Cancellable {
	...
	public xEssentialsOfflinePlayer[] getCalledPlayers() {
		...
		for(int i = 0; i < CalledPlayerNames.length; i++) {
			if(!xEssentials.getOfflinePlayer(CalledPlayerNames[i]).equals(null) || xEssentials.getOfflinePlayer(CalledPlayerNames[i]) != null) {
				offPlayers.add(xEssentials.getOfflinePlayer(CalledPlayerNames[i]));
			}
		}
