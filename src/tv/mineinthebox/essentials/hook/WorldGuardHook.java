...
public class WorldGuardHook {
	...
	public static String sendJoinMessage(Player p ) {
		...
		if(p.getPlayer().hasPermission("xEssentials.isStaff")) {
			for(ProtectedRegion region : wg.getRegionManager(p.getPlayer().getWorld()).getApplicableRegions(p.getPlayer().getLocation())) {
				if(region.getFlag(DefaultFlag.MOB_SPAWNING) == State.DENY) {
					...
					if(p.getName().equalsIgnoreCase("Xeph0re")) {
						message = ChatColor.GRAY + "a safe Developer of xEssentials " + ChatColor.GREEN + p.getPlayer().getName() + ChatColor.GRAY + " has been appeared!";	
					} else {
						message = ChatColor.GRAY + "a safe staff member " + ChatColor.GREEN + p.getPlayer().getName() + ChatColor.GRAY + " has been appeared!";	
					}
					return message;
				}
			}
			String message;
			if(p.getName().equalsIgnoreCase("Xeph0re")) {
				message = ChatColor.GRAY + "a wild Developer of xEssentials " + ChatColor.GREEN + p.getPlayer().getName() + ChatColor.GRAY + " has been appeared!";	
			} else {
				message = ChatColor.GRAY + "a wild staff member " + ChatColor.GREEN + p.getPlayer().getName() + ChatColor.GRAY + " has been appeared!";	
			}
			return message;
		} else {
			for(ProtectedRegion region : wg.getRegionManager(p.getPlayer().getWorld()).getApplicableRegions(p.getPlayer().getLocation())) {
				if(region.getFlag(DefaultFlag.MOB_SPAWNING) == State.DENY) {
					String message = ChatColor.GRAY + "a safe " + ChatColor.GREEN + p.getPlayer().getName() + ChatColor.GRAY + " has been appeared!";
					return message;
				}
			}
			String message = ChatColor.GRAY + "a wild " + ChatColor.GREEN + p.getPlayer().getName() + ChatColor.GRAY + " has been appeared!";
			return message;
		}
	}
	...
	public static String sendQuitMessage(Player p) {
		...
		if(p.getPlayer().hasPermission("xEssentials.isStaff")) {
			for(ProtectedRegion region : wg.getRegionManager(p.getPlayer().getWorld()).getApplicableRegions(p.getPlayer().getLocation())) {
				if(region.getFlag(DefaultFlag.MOB_SPAWNING) == State.DENY) {
					String message = ChatColor.RED + "Whoosh!" + ChatColor.GRAY + " staff member " + ChatColor.GREEN + p.getPlayer().getName() + ChatColor.GRAY + " has left the game safely!";
					return message;
				}
			}
			String message = ChatColor.RED + "Whoosh!" + ChatColor.GRAY + " staff member " + ChatColor.GREEN + p.getPlayer().getName() + ChatColor.GRAY + " has left the game in wild!";
			return message;
		} else {
			for(ProtectedRegion region : wg.getRegionManager(p.getPlayer().getWorld()).getApplicableRegions(p.getPlayer().getLocation())) {
				if(region.getFlag(DefaultFlag.MOB_SPAWNING) == State.DENY) {
					String message = ChatColor.RED + "Whoosh! " + ChatColor.GREEN + p.getPlayer().getName() + ChatColor.GRAY + " has left the game safely!";
					return message;
				}
			}
			String message = ChatColor.RED + "Whoosh! " + ChatColor.GREEN + p.getPlayer().getName() + ChatColor.GRAY + " has left the game in wild!";
			return message;
		}
	}
